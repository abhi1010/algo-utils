...
# %%
tickers = pd.read_csv('data-teddy-kokar/tickers.csv', header=None)[1].tolist()
...
datas = [
    bt.feeds.GenericCSVData(
        fromdate=start,
        todate=end,
        dataname=f"data-teddy-kokar/{ticker}.csv",
        dtformat=('%Y-%m-%d'),
        openinterest=-1,
        nullvalue=0.0,
        plot=False)
    for ticker in tickers
    if os.path.exists(f'data-teddy-kokar/{ticker}.csv')
]
...
class CrossSectionalMR(bt.Strategy):
  ...
  def next(self):
    available = list(
        filter(lambda d: len(d),
               self.datas))  # only look at data that existed yesterday
...
# %%
dd, cagr, sharpe = backtest(
    datas, CrossSectionalMR, plot=True, num_positions=100)
...
# %%
dd, cagr, sharpe = backtest(
    datas, CrossSectionalMR, plot=True, num_positions=20)
print(f"Max Drawdown: {dd:.2f}%\nAPR: {cagr:.2f}%\nSharpe: {sharpe:.3f}")
  ...
  def next(self):
    available = list(
        filter(lambda d: len(d) > 5,
               self.datas))  # only look at data that existed last week
    rets = np.zeros(len(available))
    ...
    selected_weights_index = np.intersect1d(
        max_weights_index, low_volality_index)
