...
def download_fpi(for_date_yyyymmdd):
  ...
  file_name = file_name_template.replace('YYYY', for_year).replace(
      'MM', for_month_num).replace('DD', for_day)

  applicable_dir = 'fpi-data'
  file_name_along_with_full_path = get_file_name_along_with_absolute_path(
      nse_data_dir_path, applicable_dir, file_name)
  ...
  if status:
    final_url = get_final_url_having_full_month_name_without_underscore(
        for_date_yyyymmdd)
    print('{:30s} | tobe downloaded !, %s'.format(file_name), final_url)
    status = download_it(final_url, file_name_along_with_full_path)
  ...
  if status and int(for_year) < 2016:
    final_url = get_final_url_having_short_month_name_and_char_v_is_missing_from_url(
        for_date_yyyymmdd)
    print('{:30s} | tobe downloaded !, %s'.format(file_name), final_url)
    status = download_it(final_url, file_name_along_with_full_path)

  if status and int(for_year) < 2016:
    final_url = get_final_url_having_full_month_name_and_char_v_is_missing_from_url(
        for_date_yyyymmdd)
    print('{:30s} | tobe downloaded !, %s'.format(file_name), final_url)
    status = download_it(final_url, file_name_along_with_full_path)

  if status and int(for_year) < 2016:
    final_url = get_final_url_having_short_month_name_and_char_l_is_missing_from_html(
        for_date_yyyymmdd)
    print('{:30s} | tobe downloaded !, %s'.format(file_name), final_url)
    status = download_it(final_url, file_name_along_with_full_path)
...
def get_final_url_having_short_month_name(for_date_yyyymmdd):
  ...
  for_day = for_date_yyyymmdd.strftime('%d')
  final_url = template_url.replace('YYYY', for_year).replace(
      'MMM', for_month_str).replace('DD', for_day)
  return final_url


def get_final_url_having_short_month_name_and_char_v_is_missing_from_url(
    for_date_yyyymmdd):
...
def get_final_url_having_short_month_name_and_char_l_is_missing_from_html(
    for_date_yyyymmdd):
...
def get_final_url_having_full_month_name_and_char_v_is_missing_from_url(
    for_date_yyyymmdd):
...
def _prepare_first_row(table1, map_of_header_sets, header_set_length, mydata):
  for j in table1.find_all('tr')[0:1]:
    ...
    for each_data in row_data:
      ...
      #     row = row + dummy
      if td_txt:
        length = len(map_of_header_sets[i + 1]) + len(
            map_of_header_sets[i + 2]) - 1
...
def file_found(file_name, cx_dir_name):
  file_name_along_with_full_path = get_file_name_along_with_absolute_path(
      nse_data_dir_path, cx_dir_name, file_name)
...
def file_present(file_name, dir_name):
  file_name_along_with_full_path = get_file_name_along_with_absolute_path(
      nse_data_dir_path, dir_name, file_name)
  if os.path.exists(file_name_along_with_full_path):
    return True
  return False
