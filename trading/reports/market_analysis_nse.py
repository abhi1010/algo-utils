...
DEFAULT_PERIOD = 'max'
SAVE_FOLDER = 'data/nse'
FILE = 'data/nse/cm01Dec2023bhav.csv'
DRY_RUN_NUM = 3
PARQUET_FILE = 'data/nse/tickers_data.parquet'
...
COLUMN_NAMES = [
    ...
    "15% up in 5 days",  #18
    "15% down in 5 days",  #19
    "25% up in 20 days",  #20
    "25% down in 20 days",  #21
    "10% above 10 DEMA",  #22
    "10% below 10 DEMA",  #23
    "Above 10 DEMA",  #24
    "Above 20 DEMA",  #25
...
python market_analysis.py --dry_run --read_parquet_file --filename data/nse/cm01Dec2023bhav.csv
...
def get_equities_data(file, dry_run):
  logger.info(f'file = {file}')
  df = pd.read_csv(file)
  if df.empty:
    logger.warning(f'empty df for file: {file}')
    return df
  logger.info(f'df = \n{df}')
  column_names = df.columns.to_list()
  # If you have a list of column names
  # Convert it to pandas Index first if it's not already
  column_names = pd.Index(column_names)  # if needed

  # Then apply the string method
  cleaned_columns = column_names.str.strip()

  # If you're working with a DataFrame, you can do:
  df.columns = cleaned_columns

  logger.info(f'column_names = {column_names}; '
              f'cleaned_columns={cleaned_columns}')
  logger.info(f'df = \n{df}')
...
def get_tickers_data(tickers,
  ...
  if dry_run:
    if read_parquet_file and os.path.exists(PARQUET_FILE):
      ...
      data = yf.download([f'{t}.NS' for t in tickers][:DRY_RUN_NUM],
                         progress=False,
                         period=f'{DRY_RUN_NUM}0d',
                         multi_level_index=False)
  else:
    data = yf.download([f'{t}.NS' for t in tickers],
                       progress=False,
                       period=DEFAULT_PERIOD,
                       multi_level_index=False)
  ...
  # only keep data from 01-Jan-2020 onwards
  data = data.loc[pd.to_datetime('2020-01-01'):]
  return data


def get_nse_indices_data(df):
  indices_info = [('^NSEI', 1), ('^CNXSC', 2), ('^CNXAUTO', 41),
                  ('^NSEBANK', 42), ('^CNXCMDT', 43), ('^CNXCONSUM', 44),
                  ('^CNXENERGY', 46), ('NIFTY_FIN_SERVICE.NS', 47),
                  ('^CNXFMCG', 48), ('^CNXINFRA', 49), ('^CNXIT', 50),
                  ('^CNXMEDIA', 51), ('^CNXMETAL', 52), ('^CNXMNC', 53),
                  ('^CNXPHARMA', 54), ('^CNXPSE', 55), ('^CNXPSUBANK', 56),
                  ('^CNXREALTY', 58), ('^CNXSERVICE', 59)]
  indices = [x[0] for x in indices_info]
  data = yf.download(indices, progress=False, period=DEFAULT_PERIOD)
...
def get_yfinance_ticker_names(nse_tickers):
  return [f'{t}.NS' for t in nse_tickers]
...
class TickDataCalculations:
  ...
  def calc_highs_and_lows(self, nse_tickers, df_raw, dry_run):
    tickers_data = self.tickers_data
  ...
  def calc_volumes(self, nse_tickers, df_raw):
    tickers_data = self.tickers_data
    simple_sum = lambda frames: frames.iloc[:, 1:].sum(axis=1)
  ...
  def check_stats(self, nse_tickers):
    yf_tickers = get_yfinance_ticker_names(nse_tickers)
...
def get_filtered_mkt_breadth_by_ts(greeks_df_file, df):
  if not os.path.exists(greeks_df_file):
    logger.warning(f'file missing. returning empty. file = {greeks_df_file}')
    return pd.DataFrame()
  ...
  existing_df = pd.read_excel(greeks_df_file,
                              ...
                              sheet_name="raw")
  logger.info(
      f'greeks_df_file={greeks_df_file}; existing_df 1 = \n{existing_df}')
  logger.info(f'df = \n{df}; shape={df.shape}')
  # existing_df = existing_df.reset_index(drop=True)

  existing_df_ts_name = 'Date/Time'
...
def get_mkt_breadth_xl_path(dry_run, add_suffix: bool):
  ...
  greeks_xl_file = os.path.join(
      SAVE_FOLDER, f"{file_prefix}market-breadth-nse{backup_suffix}.xlsx")
...
def save_local_tick_data(tickers_data):
  ...
  for i in range(num_batches):
    ...
    batch_data.to_excel(
        f'data/nse/tickers_data_batch_{i + 1}.xlsx',
        index=False)  # Save each batch to a separate Excel file


def run_analysis(args, filename):
  df = get_equities_data(filename, args.dry_run)
  tickers = list(set(df['SYMBOL'].to_list()))
  logger.info(f'ra: tickers  = {tickers}; df = \n{df}')
  logger.info(f'df = {df}')
...
def main():
  ...
  if args.date:
    ...
    try:
      bhav_input_date_fmt = '%d%m%Y'
      date_obj = datetime.datetime.strptime(args.date, "%Y%m%d")

      expected_file_part_name = date_obj.strftime(
          "sec_bhavdata_full_%d%m%Y.csv")
      expected_file_name = os.path.join(SAVE_FOLDER, expected_file_part_name)
      logger.info(f'expected_file_name = {expected_file_name}')

      if not os.path.exists(expected_file_name):
        download_bhav.download_bhav_full(
            date_obj.date().strftime('%d%m%Y'),
            dir_to_save_into=SAVE_FOLDER,
            expected_file_name=expected_file_part_name)
      else:
        logger.warning(f'File {expected_file_name} exists. Skipping download.')
      logger.info(f'date_obj={date_obj}; expected_file={expected_file_name}')
      filename = expected_file_name
    except ValueError:
      logger.info("Incomplete date. Expected format: 20231231 YYYYMMDD")
